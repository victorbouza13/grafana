...
describe('Resolver', () => {
  it('should resolve latest when no version is provided', () => {
    const pages = resolveSelectors<VersionedPages>(versionedPages);
    const components = resolveSelectors<VersionedComponents>(versionedComponents);
  ...
  it('should resolve right selector versions when an older grafana version is provided', () => {
    const pages = resolveSelectors<VersionedPages>(versionedPages, '10.0.14');
    const components = resolveSelectors<VersionedComponents>(versionedComponents, '10.0.14');
  ...
  it('should throw error if an invalid semver range is used in versioned selector', () => {
    expect(() =>
      resolveSelectors<VersionedPages>({
        Alerting: {
          AddAlertRule: {
            url: {
              abc: '/alerting/new',
